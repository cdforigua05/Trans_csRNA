import scanpy as sc
import numpy as np
import pandas as pd
import os
import h5py
import scipy as sp
import scanpy as sc
import pandas as pd
import utils as utils

def zero_ratio(X):
    unique, counts = np.unique(X, return_counts=True)
    counts = dict(zip(unique, counts))
    return counts[0]/(X.shape[0]*X.shape[1])


def read_clean(data):
    assert isinstance(data, np.ndarray)
    if data.dtype.type is np.bytes_:
        data = utils.decode(data)
    if data.size == 1:
        data = data.flat[0]
    return data

def dict_from_group(group):
    assert isinstance(group, h5py.Group)
    d = utils.dotdict()
    for key in group:
        if isinstance(group[key], h5py.Group):
            value = dict_from_group(group[key])
        else:
            value = read_clean(group[key][...])
        d[key] = value
    return d

def read_data(filename, sparsify = False, skip_exprs = False):
    with h5py.File(filename, "r") as f:
        obs = pd.DataFrame(dict_from_group(f["obs"]), index = utils.decode(f["obs_names"][...]))
        var = pd.DataFrame(dict_from_group(f["var"]), index = utils.decode(f["var_names"][...]))
        uns = dict_from_group(f["uns"])
        if not skip_exprs:
            exprs_handle = f["exprs"]
            if isinstance(exprs_handle, h5py.Group):
                mat = sp.sparse.csr_matrix((exprs_handle["data"][...], exprs_handle["indices"][...],
                                               exprs_handle["indptr"][...]), shape = exprs_handle["shape"][...])
            else:
                mat = exprs_handle[...].astype(np.float32)
                if sparsify:
                    mat = sp.sparse.csr_matrix(mat)
        else:
            mat = sp.sparse.csr_matrix((obs.shape[0], var.shape[0]))
    return mat, obs, var, uns

def prepro(data_path):
    mat, obs, var, uns = read_data(data_path, sparsify=False, skip_exprs=False)
    if isinstance(mat, np.ndarray):
        X = np.array(mat)
    else:
        X = np.array(mat.toarray())
    cell_name = np.array(obs["cell_type1"])
    cell_type, cell_label = np.unique(cell_name, return_inverse=True)
    return X, cell_label, var

def load_dataset(args):
    adata = None
    if args.dataset=="10X_PBMC":
        adata = sc.read_10x_mtx(args.data_file, var_names='gene_symbols', cache=True)
        adata.X = adata.X.toarray()
        Y = pd.read_csv(os.path.join(args.data_file, "clusters.csv"))
        Y = np.asarray(Y.Cluster)
        assert adata.X.shape[0] == Y.shape[0], "Size don't match"

        adata.obs["Group"] = Y
    if args.dataset=="HumanKidney":
        X, Y, var = prepro(args.data_file)
        adata = sc.AnnData(X)
        adata.obs['Group'] = Y
        adata.var['gene_ids'] = var.index
    if args.dataset=="RetinaMacosko":
        data_mat = h5py.File(args.data_file)
        x = np.array(data_mat['X'])
        y = np.array(data_mat['Y'])
        data_mat.close()
        adata = sc.AnnData(x)
        adata.obs['Group'] = y
    if args.dataset=="Retinaschekar":
        data_mat = h5py.File(args.data_file)
        x = np.array(data_mat['X'])
        y = np.array(data_mat['Y'])
        data_mat.close()
        adata = sc.AnnData(x)
        adata.obs['Group'] = y
    if args.dataset=="HumanLiver" or args.dataset=="WormNeuron" or args.dataset=="MouseBladder" :
        data_mat = h5py.File(args.data_file)
        x = np.array(data_mat['X'])
        y = np.array(data_mat['Y'])
        data_mat.close()
        adata = sc.AnnData(x)
        adata.obs['Group'] = y
    print(zero_ratio(adata.X))
    return adata

        

    
    return dataset
