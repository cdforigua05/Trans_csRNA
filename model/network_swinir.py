...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 1, 3, 2).contiguous().view(-1, window_size, C)
    return windows
...
def window_reverse(windows, window_size, H):
    ...
    x = x.permute(0, 1, 3, 2).contiguous().view(B, H, -1)
    return x
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size - 1), num_heads))  # 2*Wh-1 , nH

        # get pair-wise relative position index for each token inside the window
        coords_h = torch.arange(self.window_size)
        coords = torch.stack(torch.meshgrid([coords_h]))  # 1, Wh
        
        coords_flatten = torch.flatten(coords, 1)  # 1, Wh
        ...
        relative_position_index = relative_coords.sum(-1)  # Wh, Wh
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size, self.window_size , -1)  # Wh,nH,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh, Wh
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        H = x_size
        img_mask = torch.zeros((1, H, 1))  # 1 H 1
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        x = x.transpose(1, 2).view(B, self.embed_dim, x_size)  # B C Ph
        return x
